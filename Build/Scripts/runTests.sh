...
SUFFIX=$(echo $RANDOM)
NETWORK="sudhaus7-wizard-${SUFFIX}"
...
case ${TEST_SUITE} in
    ...
    checkRst)
        if [[ -d "Documentation" ]]; then
            ...
            echo ">> No documentation. Skip check."
            SUITE_EXIT_CODE=0
        fi
        ;;
    checkExceptionCodes)
    ...
    lintTypoScript)
        ...
        ${CONTAINER_BIN} run ${CONTAINER_COMMON_PARAMS} --name lint-typoscript-${SUFFIX} -e COMPOSER_CACHE_DIR=.Build/.cache/composer -e COMPOSER_ROOT_VERSION=${COMPOSER_ROOT_VERSION} ${IMAGE_PHP} /bin/sh -c "${COMMAND}"
        SUITE_EXIT_CODE=$?
        ;;
    renderDocumentation)
